        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {data.places.map((place, index) => (
                    <div key={index} style={{ border: '1px solid var(--border)', borderRadius: '2rem', backgroundColor: 'var(--surface)', overflow: 'hidden', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)', transition: 'all 0.2s ease' }}>
                        <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '1.5rem' }}>
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end' }}>
                                        <input
                                            style={{ backgroundColor: 'transparent', border: 'none', borderBottom: '2px solid var(--border)', fontSize: '1.875rem', fontWeight: 'bold', color: 'var(--foreground)', width: '100%', outline: 'none', padding: '0.25rem 0' }}
                                            placeholder="Name of Hidden Gem"
                                            value={place.name}
                                            onChange={(e) => updatePlace(index, "name", e.target.value)}
                                        />
                                        <input
                                            style={{ backgroundColor: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: '0.75rem', padding: '0.5rem 1rem', fontSize: '0.875rem', color: '#eab308', fontWeight: '600', width: '180px' }}
                                    ...
                                    <textarea
                                        ...
                                        style={{ backgroundColor: 'var(--input-bg)', minHeight: '100px', fontSize: '0.925rem', color: 'var(--foreground)' }}
                                ...
                                <button
                                    ...
                                    style={{ padding: '0.5rem', background: 'transparent', border: 'none', color: 'var(--gray-400)', cursor: 'pointer', borderRadius: '0.75rem', transition: 'background 0.2s' }}
                                >
                                    <Trash2 size={24} />
                                </button>
                            </div>

                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    <label style={{ fontSize: '0.7rem', fontWeight: 'bold', color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                        <Clock size={12} style={{ color: '#3b82f6' }} /> BEST TIME TO VISIT
                                    </label>
                                    <input
                                        className="form-input"
                                        style={{ backgroundColor: 'var(--input-bg)', fontSize: '0.875rem', color: 'var(--foreground)' }}
                ...
                {data.places.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed var(--border)', borderRadius: '1.5rem', backgroundColor: 'var(--input-bg)' }}>
                        <Star size={48} style={{ margin: '0 auto 1.5rem', color: 'var(--border)' }} />
                        <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--gray-400)', marginBottom: '0.5rem' }}>No Secrets Revealed Yet</h3>
                        <p style={{ color: 'var(--gray-400)', maxWidth: '400px', margin: '0 auto 2rem' }}>
                ...
                {data.places.length > 0 && (
                    <button
                        onClick={addPlace}
                        style={{ width: '100%', padding: '1.5rem', border: '2px dashed var(--border)', borderRadius: '1.5rem', backgroundColor: 'transparent', color: 'var(--gray-400)', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', cursor: 'pointer' }}
                    >
