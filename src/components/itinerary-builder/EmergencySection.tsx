    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
                ...
                {/* Emergency Numbers */}
                <div style={{ padding: '2.25rem', border: '1px solid var(--border)', borderRadius: '1.5rem', backgroundColor: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: '1.5rem', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                        ...
                        {data.emergencyNumbers.map((entry, i) => (
                            <div key={i} style={{ display: 'flex', gap: '0.5rem' }}>
                                <input
                                    ...
                                    style={{ backgroundColor: 'var(--input-bg)', flex: 1, fontSize: '0.875rem', color: 'var(--foreground)' }}
                                ...
                                <input
                                    className="form-input"
                                    style={{ backgroundColor: 'var(--input-bg)', width: '120px', fontSize: '0.875rem', color: 'var(--foreground)' }}
                                    placeholder="Number"
                                    value={entry.number}
                                    onChange={(e) => updateNumber(i, "number", e.target.value)}
                                />
                                <button onClick={() => removeNumber(i)} style={{ padding: '0.5rem', background: 'transparent', border: 'none', color: 'var(--gray-400)', cursor: 'pointer' }}>
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ))}
                ...
                <div style={{ padding: '2.25rem', border: '1px solid var(--border)', borderRadius: '1.5rem', backgroundColor: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: '1.5rem', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <div style={{ padding: '1.25rem', borderRadius: '1rem', backgroundColor: 'var(--input-bg)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                <div>
                                    <h4 style={{ fontSize: '0.925rem', fontWeight: 'bold', color: 'var(--foreground)', margin: 0 }}>Kyoto University Hospital</h4>
                                    <p style={{ fontSize: '0.75rem', color: 'var(--gray-400)', margin: '0.25rem 0 0 0' }}>Major center with English support.</p>
                                </div>
                                <MapPin size={16} style={{ color: '#3b82f6' }} />
                            </div>
                            <p style={{ fontSize: '0.825rem', color: 'var(--foreground)', margin: 0 }}>54 Shogoin Kawaharacho, Sakyo Ward, Kyoto.</p>
                        </div>
                        <div style={{ padding: '1.25rem', borderRadius: '1rem', backgroundColor: 'var(--input-bg)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                <div>
                                    <h4 style={{ fontSize: '0.925rem', fontWeight: 'bold', color: 'var(--foreground)', margin: 0 }}>Japan Health Info (Service)</h4>
                                    <p style={{ fontSize: '0.75rem', color: 'var(--gray-400)', margin: '0.25rem 0 0 0' }}>Call to find the nearest clinic.</p>
                                </div>
                                <ExternalLink size={16} style={{ color: '#3b82f6' }} />
                            </div>
                            <p style={{ fontSize: '0.825rem', color: 'var(--foreground)', margin: 0 }}>Website: japanhealthinfo.com</p>
                        </div>
                    </div>
                </div>

                {/* Insurance Info */}
                <div style={{ gridColumn: '1 / -1', padding: '2.5rem', border: '1px solid var(--border)', borderRadius: '1.5rem', backgroundColor: 'var(--input-bg)', borderLeft: '6px solid #10b981', display: 'flex', gap: '1.5rem', alignItems: 'center', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                    <div style={{ padding: '1rem', borderRadius: '100%', backgroundColor: 'var(--surface)', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)' }}>
                        <AlertCircle size={28} />
                    </div>
                    <div>
                        <h3 style={{ fontSize: '1.125rem', fontWeight: 'bold', color: 'var(--foreground)', margin: '0 0 0.5rem 0' }}>Travel Insurance Quick-Reference</h3>
                        <p style={{ fontSize: '0.925rem', color: 'var(--gray-400)', margin: 0, lineHeight: 1.5 }}>
